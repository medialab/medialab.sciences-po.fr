...
$posttypes = (array) get_post_types( array('show_ui' => true, '_builtin' => false, 'show_in_menu' => true ) );
ksort($posttypes);
foreach ( $posttypes as $ptype ) {
	...
	if ( $ptype_obj->show_in_menu !== true )
		continue;
	$ptype_menu_position = 6; //is_int( $ptype_obj->menu_position ) ? $ptype_obj->menu_position : ++$_wp_last_object_menu; // If we're to use $_wp_last_object_menu, increment it first.
	...
	$core_menu_positions = array(6, 7, 8, 9);
...
$menu[3] = array( '', 'read', 'separator1', '', 'wp-menu-separator' );
...
/*	$submenu['edit.php'][10]  = array( _x('Add New', 'post'), get_post_type_object( 'post' )->cap->create_posts, 'post-new.php' );
	...
	unset($tax);
*/
$menu[20] = array( __('Media'), 'upload_files', 'upload.php', '', 'menu-top menu-icon-media', 'menu-media', 'none' );
...
*/
$menu[5] = array( __('Pages'), 'edit_pages', 'edit.php?post_type=page', '', 'menu-top menu-icon-page', 'menu-pages', 'none' );
...
$menu[75] = array( __('WPTools'), 'edit_posts', 'tools.php', '', 'menu-top menu-icon-tools', 'menu-tools', 'none' );
