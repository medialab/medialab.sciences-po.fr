...
<div class="container">
	<div id="content">
		...
		while ( $loop->have_posts() ) : $loop->the_post();
			...
			$el = '<div class="column_display publications '.get_object_terms('projets').' '.get_object_terms('people').' '.get_object_terms('publications_types').'"><h2><a href="'.get_permalink($post->id).'">'.$post->post_title.'</a></h2><h3><a href=" '.get_object_terms('publications_types').'">'.implode($terms, ',').'</a></h3><div id="img_publi"><a href=" '.get_permalink($post->id).'">'.get_the_post_thumbnail().'</a></div><p>'.get_shorten_excerpt(get_the_excerpt(), 140).'</p></div>';
